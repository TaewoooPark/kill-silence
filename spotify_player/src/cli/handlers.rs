...
pub fn handle_cli_subcommand(cmd: &str, args: &ArgMatches) -> Result<()> {
    ...
    match cmd {
        "authenticate" => {
            // KILL//SILENCE uses external Spotify Connect playback, so one Web API approval is
            // sufficient. Integrated librespot authentication remains an explicit streaming-only
            // concern and is deliberately not triggered here.
