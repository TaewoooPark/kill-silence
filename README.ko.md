...
<p align="center"><strong>에이전트가 내 직업을 대체할까 걱정되십니까? 우울하게 대체될 바에 저는 노래나 듣겠습니다.</strong></p>
...
에이전트가 코드를 쓰는 동안 우리는 무엇을 할까요? 진지하게 로그를 읽으십니까? 물론 아닐 것입니다. 이러다 우리는 에이전트에게 모든 직업을 빼앗기지는 않을까요? 우리는 그렇다면 무엇을 해야 할까요.

솔직히 전 잘 모르겠습니다. 그냥 노래나 듣고 싶습니다. 그래서 **KILL//SILENCE**를 만들었습니다. with-agents 커멘드를 사용하면, Claude Code가 일하는 동안 음악을 틀어줍니다. 노래가 멈추면 다시 프롬프트를 입력하면 됩니다. Claude가 다시 일하기 시작하면 노래가 다시 흘러나옵니다. 당신의 Spotify 계정을 연동해서, 세상 만사 귀찮은 사람들의 에이전틱 코딩을 경험하세요.

<p align="center">
  <img src="./docs/screenshots/01-main.png" alt="KILL//SILENCE 메인 터미널 화면" width="100%">
</p>
...
커맨드 창에서 `/`만 치면 아래에 인덱스가 뜹니다. `↑` / `↓`로 고르고 `Tab` 또는 `→`로 완성한 뒤 `Enter`를 누르세요.
...
팝업이나 알림음으로 오는 에이전트의 종료 신호는 너무 딱딱합니다. 저는 노래나 듣고 싶습니다. 노래가 멈추면 노래를 다시 듣기 위해 일할 뿐입니다.
...
|---|---|
| ![Spotify 플레이어 아카이브](./docs/screenshots/02-player.png) | ![Claude 작업 중](./docs/screenshots/03-agent-working.png) |

| 03 · 음악이 멈추면 돌아온다 | 04 · 세션을 고른다 |
|---|---|
| ![에이전트 작업 완료](./docs/screenshots/04-agent-complete.png) | ![Claude 세션 선택](./docs/screenshots/05-claude-session-picker.png) |

| 05 · Claude 옆에서 플레이 | 06 · Claude 옆의 홈 |
|---|---|
| ![Claude 옆에서 실행 중인 Spotify 플레이어](./docs/screenshots/06-player-running.png) | ![Claude Code 옆 KILL//SILENCE 홈](./docs/screenshots/07-claude-home.png) |
